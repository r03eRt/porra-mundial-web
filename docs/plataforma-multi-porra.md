...
| 1 🚧 | Dashboard MVP: crear porra, configurar y operar manualmente | En curso: app `admin-next/` (Vite + supabase-js). Hecho: login admin, crear/listar porras, borrar borradores, detalle por porra, asistente guiado para crear grupos/equipos por pasos y generar los partidos al final con fecha opcional, mini-porra editable, generación y reset automático de partidos de grupo por jornadas con fecha inicial opcional y días entre jornadas, asistente que permanece visible/plegable/editable tras guardar, secciones de jugadores/partidos/mini-porra plegables, edición inline de la fecha/hora de cada partido, reaparición del asistente al reset para regenerar la misma fase de grupos, orden manual de partidos con botones y arrastre en fase de grupos, y ciclo `draft → open → playing → closed` |
