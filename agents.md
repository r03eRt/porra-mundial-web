...
- ✅ **Máximos goleadores** (`renderTopScorers`/`calculateTopScorers`): agrega `porra_matches.scorers` (jsonb, parseado con el `goalBreakdown` ya existente) en un ranking global de hasta 15 goleadores, ignorando autogoles, replicando la `calculateTopScorers` de la legacy. Tabla Pos./Jugador/Selección (con bandera)/Goles. Si el organizador aún no ha metido goleadores, muestra empty-state
